...
with st.sidebar:
    ...
    with st.expander("🎯 Grupo de Maturidade", expanded=False):
        gm_min = float(ta_f9["gm_cat"].min()) if not ta_f9.empty else 5.5
        gm_max = float(ta_f9["gm_cat"].max()) if not ta_f9.empty else 8.5
        gm_min = max(5.5, round(gm_min, 1))
        gm_max = min(8.5, round(gm_max, 1))
        if gm_min >= gm_max:
            gm_max = min(gm_min + 0.1, 8.5)
        gm_range = st.slider("GM", min_value=5.5, max_value=8.5, value=(gm_min, gm_max), step=0.1, format="%.1f")
